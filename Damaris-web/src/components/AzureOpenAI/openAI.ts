...
export async function azureOpenAICompletion(
	request: string,
	reformatMessage: boolean,
	userTone: string
) {
	try {
		...
		const consideratePrompt = reformatMessage
			? await reformatQNAPrompt(userTone, request)
			: await reformatSystemPrompt(userTone, request);

		console.log("New prompt to customer: " + consideratePrompt);

		// Generate the final response using Azure OpenAI
		const completions = await client.getCompletions(openAIDeployment, [
			consideratePrompt,
		]);
...
export async function azureOpenAIChat(request: string) {
	...
	const systemContext =
		"You are an experienced customer representative for a shipping company. " +
...
function reformatSystemPrompt(userTone: string, request: string): string {
	...
	const prompt =
        ...
        Considering that your customer is feeling` +
		userTone +
		`, ` +
